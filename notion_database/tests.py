...
class TestGetAllUserNotionDatabaseProperties(TestDatabaseResponseConversion):
    ...
    def test_notion_db_api_object_converted_to_simple_python_dictionary(self, m):
        self.client.force_login(get_user_model().objects.get_or_create(username=self.user.username)[0])
        db_dict_list = query_user_notion_databases_list(user_model=self.user, query_string='')
        for db_dict in db_dict_list:
            if not 'name' in db_dict or not 'properties' in db_dict or not 'id' in db_dict:
                raise Exception('Invalid conversion of a dictioanry, proeprties are missing!')
            for property_container in db_dict['properties']:
                if property_container.notion_type in IGNORED_PROPERTIES_SET:
                    raise Exception(f'Property {property_container.notion_type} was supposed to be ignored!')
...
class TestGetSingleDatabaseProperties(TestDatabaseResponseConversion):
    ...
    def test_convert_query_user_notion_database_by_id_api_resp_to_dictionary(self, m):
        self.client.force_login(get_user_model().objects.get_or_create(username=self.user.username)[0])
        db_dict = query_user_notion_database_by_id(self.user, VALID_DATABASE_ID)
        for property_container in db_dict['properties']:
            if property_container.notion_type in IGNORED_PROPERTIES_SET:
                raise Exception(f'Property {property_container.notion_type} was supposed to be ignored!')
